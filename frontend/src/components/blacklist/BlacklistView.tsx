...
export const BlacklistView: React.FC<BlacklistViewProps> = ({
  ...
  // State management: DB-backed blacklist
  ...
  // Fetch real blacklist entries from PostgreSQL API
    ...
    try {
      ...
      if (res.ok) {
        ...
        const mapped: BlacklistEntry[] = (data.entries || []).map((e: any) => ({
          id: e.blacklist_id,
          plateNumber: e.plate_text,
          reason: e.reason,
          dateAdded: e.date_added_formatted || new Date(e.added_at).toLocaleDateString(),
          timeAdded: e.time_added_formatted || new Date(e.added_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
          lastFound: e.last_found || 'Never',
          cameraName: e.last_camera_name,
          location: e.last_location,
          active: e.active,
        }));
        setEntries(mapped);
      }
  ...
  const handleAddNewVehicle = async (
    ...
    try {
      const res = await fetch('/blacklist', {
        ...
        body: JSON.stringify({
          plate_text: newVehicle.plateNumber,
          reason: newVehicle.reason,
        }),
      });

      if (res.ok) {
        setSuccessToast(`Vehicle ${newVehicle.plateNumber} added to Blacklist`);
        setTimeout(() => setSuccessToast(null), 3500);
        await fetchBlacklist();
        setCurrentPage(1);
      } else {
        const errorData = await res.json().catch(() => ({}));
        setSuccessToast(`Error: ${errorData.detail || 'Could not add vehicle to blacklist'}`);
        setTimeout(() => setSuccessToast(null), 4000);
      }
    } catch (err) {
      console.error('Failed to add vehicle to blacklist:', err);
      setSuccessToast('Network error while adding vehicle');
      setTimeout(() => setSuccessToast(null), 3500);
    }
  };
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 select-none pt-2">
          ...
          <div className="flex items-center gap-2 select-none">
            <span className="text-xs md:text-sm font-bold font-body text-[#AC251D] tracking-wider uppercase">
              BLACKLISTED VEHICLES: {filteredEntries.length}
            </span>
          </div>
