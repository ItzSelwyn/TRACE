...
export const AlertsView: React.FC = () => {
  ...
  // Fetch real alerts from PostgreSQL backend API
  const fetchAlerts = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        const mapped: AlertItem[] = (data.alerts || []).map((a: any) => ({
          id: a.alert_id,
          plateNumber: a.plate_text,
          type: a.category === 'BLACKLIST' ? 'BLACKLIST' : 'ANOMALY',
          status: a.status === 'VERIFIED' ? 'VERIFIED' : 'UNVERIFIED',
          confidence: a.confidence ?? 92,
          vehicleType: a.vehicle_type || 'SUV',
          vehicleColor: a.vehicle_color || 'RED',
          scannedTimestamp: a.scanned_timestamp || new Date(a.triggered_at).toLocaleTimeString(),
          cameraName: a.camera_name || 'Camera 16',
          location: a.location || 'North Highway 02',
          reason: a.reason,
        }));
        setAlerts(mapped);
      }
  ...
  useEffect(() => {
    fetchAlerts();
    // Poll every 10 seconds for real-time camera alerts
  ...
  // Compute live dynamic Verified & Unverified counts
  ...
  // Handle Verify Action (updates PostgreSQL in real time)
  const handleVerifyAlert = async (alertId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        console.error('Failed to update alert verification in database');
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto pb-6 select-none relative">
      ...
      <div className="space-y-4">
        {isLoading && alerts.length === 0 ? (
          <div className="bg-[#151515] rounded-[3px] p-12 text-center text-[#A0A0A0] font-body flex flex-col items-center justify-center gap-3">
            <div className="w-6 h-6 border-2 border-[#AC251D] border-t-transparent rounded-full animate-spin" />
            <p className="text-xs uppercase tracking-wider font-semibold">Loading real-time alerts from database...</p>
          </div>
        ) : filteredAlerts.length === 0 ? (
          <div className="bg-[#151515] rounded-[3px] p-12 text-center text-[#A0A0A0] font-body">
