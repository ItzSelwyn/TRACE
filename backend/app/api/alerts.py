...
logger = logging.getLogger("trace.api.alerts")
...
def _get_fallback_alerts(
    status_filter: Optional[str] = None,
    category_filter: Optional[str] = None,
    q: Optional[str] = None,
) -> AlertListResponse:
    """Provide realistic baseline alerts when PostgreSQL is offline or authenticating."""
    now = datetime.now(timezone.utc)
    fallback_items = [
        AlertResponse(
            alert_id=uuid.UUID("11111111-0000-0000-0000-000000000001"),
            type="blacklist_hit",
            plate_text="TN 37 CY 1234",
            camera_id=uuid.UUID("c4000000-0000-0000-0000-000000000004"),
            triggered_at=now,
            reviewed=False,
            ...
            vehicle_color="BLUE",
            confidence=92,
            scanned_timestamp=_format_scanned_timestamp(now),
            reason="Tracing vehicle's trajectory",
            category="BLACKLIST",
            status="UNVERIFIED",
        ),
        AlertResponse(
            alert_id=uuid.UUID("22222222-0000-0000-0000-000000000002"),
            type="impossible_journey",
            plate_text="TN 57 CY 1314",
            camera_id=uuid.UUID("c1000000-0000-0000-0000-000000000001"),
            triggered_at=now,
            reviewed=False,
            ...
            confidence=84,
            scanned_timestamp=_format_scanned_timestamp(now),
            reason="Implied speed 142 km/h exceeds corridor threshold (120 km/h)",
            category="ANOMALY",
            status="UNVERIFIED",
        ),
        AlertResponse(
            alert_id=uuid.UUID("33333333-0000-0000-0000-000000000003"),
            type="camera_inconsistency",
            plate_text="TN 57 CY 1314",
            camera_id=uuid.UUID("c4000000-0000-0000-0000-000000000004"),
            triggered_at=now,
        ...
        AlertResponse(
            alert_id=uuid.UUID("44444444-0000-0000-0000-000000000004"),
            type="blacklist_hit",
            plate_text="TN 57 CY 1314",
            camera_id=uuid.UUID("c2000000-0000-0000-0000-000000000002"),
            triggered_at=now,
            reviewed=True,
            reviewed_at=now,
    ...
    for item in fallback_items:
        if status_filter and status_filter.upper() != "ALL" and item.status != status_filter.upper():
            continue
        if category_filter and category_filter.upper() != "ALL" and item.category != category_filter.upper():
            continue
        if q and q.strip():
    ...
    verified_count = sum(1 for a in fallback_items if a.reviewed)
    unverified_count = sum(1 for a in fallback_items if not a.reviewed)
...
async def get_alerts(
    ...
    try:
        ...
        logger.warning(f"[DB WARNING] Database connection offline or auth failed in get_alerts ({e}). Returning fallback alerts.")
        return _get_fallback_alerts(status_filter, category_filter, q)
...
async def update_alert(
    ...
    try:
        ...
        logger.warning(f"[DB WARNING] Database connection offline or auth failed in update_alert ({e}). Returning fallback response.")
        return AlertResponse(
            alert_id=id,
            type="blacklist_hit",
            plate_text="TN 37 CY 1234",
